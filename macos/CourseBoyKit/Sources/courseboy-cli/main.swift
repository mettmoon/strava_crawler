...
//   courseboy --tcx route.tcx --segments route_segments.json [--min-category 4] [--out out.tcx]
...
guard let tcxPath = arg("--tcx"), let segPath = arg("--segments") else {
    FileHandle.standardError.write(Data("usage: courseboy --tcx <tcx> --segments <json> [--min-category 4] [--out <tcx>]\n".utf8))
    exit(2)
}
